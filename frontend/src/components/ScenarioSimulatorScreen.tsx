...
export function ScenarioSimulatorScreen() {
  ...
  const riskLevel =
    paymentToAvailableRatio < 30
      ? "low"
      : paymentToAvailableRatio < 50
      ? "medium"
      : "high";
        ...
        <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
          <div className="text-center mb-4">
            ...
            <p className="text-sm text-gray-500 mt-1">
              per month for {loanTerm[0]} months
            </p>
          </div>

          <div
            className={`${riskColors.bg} ${riskColors.border} border rounded-xl p-4 flex items-center gap-3`}
          >
            {isAffordable ? (
            ...
            <div className="flex-1">
              <p className={`text-sm ${riskColors.text}`}>
                {isAffordable
                  ? `This payment is within your budget (${paymentToAvailableRatio.toFixed(
                      0
                    )}% of available income)`
                  : `This payment may strain your budget (${paymentToAvailableRatio.toFixed(
                      0
                    )}% of available income)`}
              </p>
            </div>
      ...
      <div className="px-6 mt-6 mb-6">
        <div className="bg-white rounded-xl p-5 border border-gray-100">
          <div className="flex items-center gap-3 mb-4">
            <div
              className={`w-10 h-10 ${riskColors.bg} rounded-lg flex items-center justify-center`}
            >
          ...
          <p className="text-sm text-gray-600">
            {riskLevel === "low" &&
              "This loan amount is comfortable for your budget with plenty of room for unexpected expenses."}
            {riskLevel === "medium" &&
              "This loan is manageable but leaves less buffer for emergencies. Consider building savings first."}
            {riskLevel === "high" &&
              "This payment may be too high for your current income. Consider a smaller loan or longer term."}
          </p>
        </div>
      </div>
      ...
      <div className="px-6 mb-6 mt-12">
        <div className="bg-gray-100 rounded-xl p-4">
          <p className="text-xs text-gray-600 leading-relaxed">
            © Copyright 2024 LoanHook. All rights reserved. Jy'Mir Fuller & Joseph Ajumobi |{" "}
